...
K_IMU = 0.085  # rad/s / deg  #0.08 el weno
...
cmd = {"vx": 0.0, "vy": 0.0, "habilitado": False}
...
def thread_ctrl():
    """
    Recibe (vx, vy) del cmd compartido.
    Lee el IMU para detectar rotación no deseada y agrega omega_imu.
    Aplica cinemática Mecanum a 100 Hz.
    """
    DT       = 0.01   # 100 Hz
    yaw_ref  = None
    yaw_init = False
    ...
    while True:
        ...
        # Inicializar referencia de heading al primer dato
        if not yaw_init:
            yaw_ref = yaw_actual
            yaw_init = True

        # El carro holonómico mantiene yaw_ref constante.
        # Cualquier desviación es una perturbación que corregimos.
        ...
        aplicar_mecanum(vx_c, vy_c, omega_imu)

        sys.stdout.write(
            f"\r[CTRL 100Hz] vx={vx_c:+.3f} vy={vy_c:+.3f}  "
            f"err_yaw={err_yaw:+.1f}°  ω_imu={math.degrees(omega_imu):+.1f}°/s    "
        )
        sys.stdout.flush()
        time.sleep(DT)

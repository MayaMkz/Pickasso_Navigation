...
EXTRA_SEGURIDAD_M = 0.15  # [NUEVO] Distancia extra para alejar el chasis antes del giro
...
def aplicar_velocidades_recto(v_izq, v_der):
    # La lógica interna de signos de esta función permite avanzar o retroceder
...
def diagnostico_inicial_motores():
    ...
    while True:
        ...
        if dif_1A < umbral_fallo or dif_1B < umbral_fallo or dif_2A < umbral_fallo or dif_2B < umbral_fallo:
            ...
            input("\n>>> Presiona ENTER cuando hayas revisado las conexiones para reintentar...")
        else:
...
# ==========================================
# 6. RUTINAS DE AVANCE, REVERSA Y GIRO
# ==========================================
def avanzar_offset(metros_extra, rpm_base=20):
    if metros_extra <= 0: return
    print(f"\n-> [AVANCE] Cubriendo {metros_extra}m extra hacia adelante...")
    ...
    while True:
        ...
        sys.stdout.write(f"\r   Faltan: {metros_extra - (recorrido / PULSOS_POR_METRO):.2f}m... ")
        sys.stdout.flush()
        time.sleep(0.02)
    print(" [OK]")

def retroceder_offset(metros_extra, rpm_base=20):
    if metros_extra <= 0: return
    print(f"\n-> [REVERSA] Retrocediendo {metros_extra}m para engancharse a la ruta...")
    ...
    while True:
        ...
        # Al enviar RPM negativas, la función invierte automáticamente los H-Bridges
        aplicar_velocidades_recto(-rpm_base - ajuste, -rpm_base + ajuste)
...
def girar_grados(grados, rpm_giro=20):
    print(f"\n-> [ROTACIÓN] Girando {grados:.1f} grados libres de obstáculos...")
    yaw_ini = obtener_yaw()
...
try:
    while True:
        ...
        # === DETECCIÓN DE ESQUINA (MANIOBRA SEGURA) ===
        if abs(dif_vision) > 45: 
            ...
            # 1. Avanzamos para alinear el Robot con la Estación (Operation Pose)
            avanzar_offset(OFFSET_CENTRO_M, rpm_base=20)
            frenar_y_limpiar()
            
            # 2. Operación del xArm5 (10 Segundos exactos en la estación)
            if contador_esquinas == 1 or contador_esquinas == 3:
                print("\n[■] OPERACIÓN EN ESTACIÓN: Esperando 10 segundos para el brazo robótico...")
                for s in range(10, 0, -1):
                    sys.stdout.write(f"\r    T-{s} segundos...   ")
                    sys.stdout.flush()
                    time.sleep(1)
                print("\n[►] Operación terminada. Preparando maniobra de giro seguro.")
            else:
                time.sleep(0.3) 
            
            # 3. Maniobra "Dogleg" para alejar el chasis de la mesa
            print("\n[+] EJECUTANDO OPCIÓN B (Evasión de Barrido)")
            avanzar_offset(EXTRA_SEGURIDAD_M, rpm_base=20)
            
            # 4. Giro libre de obstáculos
            grados_a_girar = 90.0 * datos_vision["sentido"]
            girar_grados(grados_a_girar, rpm_giro=20)
            
            # 5. Retorno en reversa a la línea ideal (Enganche)
            retroceder_offset(EXTRA_SEGURIDAD_M, rpm_base=20)
            
            # Reset de variables para la nueva recta
            angulo_vision_anterior = angulo_vision_actual
            yaw_recta_base = obtener_yaw() 
            yaw_objetivo_local = yaw_recta_base
            continue

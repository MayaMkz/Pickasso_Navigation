...
try:
    while True:
        if not datos_vision["activo"] or (time.time() - datos_vision["ultima_vez"] > 1.0):
            frenar_y_limpiar()
            angulo_vision_anterior = None
            # Reseteamos el contador para cuando inicie una nueva vuelta (Presionen 's' en la PC)
            contador_esquinas = 0 
        ...
        if abs(dif_vision) > 45: 
            frenar_y_limpiar()
            contador_esquinas += 1 # Sumamos una esquina
            ...
            # --- LÓGICA DE PAUSA (10s en Tag 1 y Tag 2) ---
            if contador_esquinas == 1 or contador_esquinas == 3:
                ...
                # Esquina virtual (esquina 2), solo pausa un instante para asentar la inercia
                time.sleep(0.3)

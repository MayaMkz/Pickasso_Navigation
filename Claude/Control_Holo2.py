...
def thread_ctrl():
    ...
    ─────────────────────
    La PC calcula omega_heading = cuánto debe girar el carro (rad/s)
    para que la flecha roja quede paralela o perpendicular al segmento.

    El IMU tiene dos modos:
    • Visión corrigiendo (|omega_heading| > umbral):
        El IMU se quita de en medio. yaw_ref sigue al yaw real para
        no acumular error. El carro gira según dice la visión.
    • Visión satisfecha (|omega_heading| ≈ 0):
        El IMU congela el ángulo logrado y lo mantiene contra
        perturbaciones (vibraciones, inercia, piso resbaloso).
    ...
    yaw_ref       = None
    yaw_init      = False
    UMBRAL_CORR   = 0.05    # rad/s — por debajo de esto el IMU toma control
    ...
    while True:
        ...
        if abs(ohd_c) > UMBRAL_CORR:
            # ── Visión corrigiendo: IMU sigue al yaw real, no pelea ──
            yaw_ref   = yaw_actual   # track real → error_imu = 0
            omega_imu = 0.0
        else:
            # ── Visión satisfecha: IMU mantiene el ángulo logrado ────

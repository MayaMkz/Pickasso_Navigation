...
def main():
    ...
    # Matriz de distorsión nula (usada porque trabajamos sobre la imagen ya corregida)
    ...
    while True:
        ...
        if ids is not None:
            for i in range(len(ids)):
                m_id = int(ids[i][0])
                # Usamos zero_dist porque la imagen 'gray' ya viene del remap (ya está sin distorsión)
                ...
                if success:
                    ...
                    # [MODIFICADO] Guardamos la profundidad m_z para la proyección 2D
        ...
        # =========================================================
        # [NUEVO] LÓGICA DE PREVISUALIZACIÓN Y DIBUJO DE RECTÁNGULO
        # =========================================================
        if 0 in memoria_tags and 1 in memoria_tags and 2 in memoria_tags:
            ...
            # Si la misión NO está activa, usamos al robot como "Home Temporal" para previsualizar
            if not mision_activa:
                hx, hy, hz = memoria_tags[0]['m_x'], memoria_tags[0]['m_y'], memoria_tags[0]['m_z']
                color_rect_radar = (100, 100, 100)  # Gris en radar
                color_rect_cam = (0, 150, 255)      # Naranja en cámara
            else:
                hx, hy = posicion_home[0], posicion_home[1]
                hz = memoria_tags[0]['m_z']
                color_rect_radar = (0, 255, 0)      # Verde en radar
                color_rect_cam = (0, 255, 0)        # Verde en cámara

            # 1. Dibujar Rectángulo en el RADAR
            puntos_circ_radar = np.array([
                metros_a_pixeles_radar(hx, hy), 
                metros_a_pixeles_radar(t1x, t1y), 
                metros_a_pixeles_radar(vx, vy), 
                metros_a_pixeles_radar(t2x, t2y)
            ], np.int32).reshape((-1, 1, 2))
            cv2.polylines(minimapa, [puntos_circ_radar], isClosed=True, color=color_rect_radar, thickness=2)

            # 2. Dibujar Rectángulo en la CÁMARA
            if camera_matrix is not None:
                # Armamos el array de puntos en coordenadas de cámara (Metros)
                pts_3d = np.array([[hx, hy, hz], [t1x, t1y, t1z], [vx, vy, vz], [t2x, t2y, t2z]], dtype=np.float32)
                # Proyectamos esos metros directamente a los pixeles de la pantalla
                pts_2d, _ = cv2.projectPoints(pts_3d, np.zeros((3,1)), np.zeros((3,1)), camera_matrix, zero_dist)
                pts_2d = np.int32(pts_2d).reshape((-1, 1, 2))
                cv2.polylines(cam_view, [pts_2d], isClosed=True, color=color_rect_cam, thickness=2)
            
            # 3. Calcular y mostrar MEDIDAS (Largo y Ancho)
            largo_m = math.hypot(t1x - hx, t1y - hy)
            ancho_m = math.hypot(vx - t1x, vy - t1y)
            texto_medidas = f"Circuito: {largo_m:.2f}m x {ancho_m:.2f}m"
            cv2.putText(cam_view, texto_medidas, (30, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_rect_cam, 2)
        # =========================================================

        # Lógica de rastreo del robot en el radar

...
def main():
    ...
    # [NUEVO] MARGEN DE SEGURIDAD (10 cm + 25 cm = 0.35m)
    OFFSET_MESA_M = 0.35
    ...
    if fs.isOpened():
        ...
        print("[-] ALERTA: No se encontró archivo. Usando parámetros genéricos.")
        w, h = 1280, 720
        focal_length = w * 0.9 
    ...
    # Variables para almacenar la ruta expandida de forma global
    ruta_pts_global = []
    ...
    while True:
        ...
        # =========================================================
        # [MODIFICADO] LÓGICA DE EXPANSIÓN DE OBSTÁCULOS (LA MESA)
        # =========================================================
        if 1 in memoria_tags and 2 in memoria_tags:
            ...
            # 1. Definimos las 4 esquinas físicas de la MESA
            cz = (t1z + t2z) / 2.0
            mesa_pts = [
                (t1x, t1y, t1z),   # Tag 1
                (t2x, t1y, cz),    # Virtual 1
                (t2x, t2y, t2z),   # Tag 2
                (t1x, t2y, cz)     # Virtual 2
            ]

            # 2. Calculamos el centro matemático de la mesa
            centro_mesa_x = (t1x + t2x) / 2.0
            centro_mesa_y = (t1y + t2y) / 2.0

            # 3. Función matemática para inflar el rectángulo empujando hacia afuera del centro
            ...
            # 4. Generamos los 4 puntos de la RUTA SEGURA (Expandida)
            ruta_pts_global = [expandir_punto(*p) for p in mesa_pts]

            # --- DIBUJOS EN EL RADAR ---
            # Polígono de la Mesa (Azul Claro)
            ...
            # Polígono de la Ruta Segura (Verde Brillante)
            ...
            # --- DIBUJOS EN LA CÁMARA (PROYECCIÓN 3D) ---
            ...
            txt_mesa = f"Mesa Fija: {ancho_mesa:.2f}m x {largo_mesa:.2f}m"
            txt_ruta = f"Ruta Carro: {ancho_ruta:.2f}m x {largo_ruta:.2f}m"
        ...
        # =========================================================
        # RASTREO Y NAVEGACIÓN DEL ROBOT
        # =========================================================
        if 0 in memoria_tags:
            ...
            # Lógica de avance de misión (USANDO LOS PUNTOS EXPANDIDOS)
            if mision_activa and len(ruta_pts_global) == 4:
                
                # Asignamos las esquinas expandidas como los verdaderos objetivos
                if estado_mision == 1: 
                    objetivo_actual = ruta_pts_global[0]; nombre_objetivo = "1. Est.1 (Margen Seguro)"
                elif estado_mision == 2: 
                    objetivo_actual = ruta_pts_global[1]; nombre_objetivo = "2. Esq. Virtual 1"
                elif estado_mision == 3: 
                    objetivo_actual = ruta_pts_global[2]; nombre_objetivo = "3. Est.2 (Margen Seguro)"
                elif estado_mision == 4: 
                    objetivo_actual = ruta_pts_global[3]; nombre_objetivo = "4. Esq. Virtual 2 (Home)"
        ...
        if tecla == ord('q'): break
            ...
            if 0 in memoria_tags and 1 in memoria_tags and 2 in memoria_tags:
                ...
                # Determinamos el giro basado en la posición inicial del robot respecto a la mesa

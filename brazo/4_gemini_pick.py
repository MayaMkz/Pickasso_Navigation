...
CAMERA_Z_OFFSET_M = 0.20  # 20 cm de altura para centrar la cámara y observar
PICK_OFFSET_Z_M   = 0.03  # 3 cm de seguridad antes del agarre

# --- PARÁMETROS CRÍTICOS A VERIFICAR FÍSICAMENTE ---
GRIPPER_LENGTH_M  = 0.170 # ¡MIDE ESTO! Distancia del link5 a la punta de los dedos.
Z_AGARRE_EXTRA_M  = 0.015 # Cuánto penetra el gripper respecto a la tapa del cubo.
...
class PickAndPlaceNode(Node):
    ...
    def _ciclo_pick(self, punto_inicial: Point, max_intentos: int = 3) -> bool:
        for intento in range(1, max_intentos + 1):
            # ──────────────────────────────────────────────────────────
            # FASE 1: ACERCAMIENTO INICIAL (Usando lectura con ángulo)
            # ──────────────────────────────────────────────────────────
            resultado_inicial = self._calcular_pose_objeto(punto_inicial)
            if not resultado_inicial: return False

            x_obj_init, y_obj_init, z_obj_init = resultado_inicial
            yaw = math.atan2(y_obj_init, x_obj_init)
            quat = quat_gripper_apuntando_abajo(yaw)
            R_target = quat_to_rot_matrix(quat)

            p_cam_target = np.array([x_obj_init, y_obj_init, z_obj_init + CAMERA_Z_OFFSET_M])
            t_cam = np.array([0.067506, 0.007342, 0.035900])
            p_brida_cam = p_cam_target - R_target @ t_cam

            self.get_logger().info('📸 FASE 1: Centrando cámara sobre el objeto...')
            if not self._plan_pose(p_brida_cam[0], p_brida_cam[1], p_brida_cam[2], quat, 'CENTRAR_CAMARA'):
                return False
            
            # Pausa interactiva para darte control en las pruebas
            print('\n═' * 55)
            ...
            # Dar tiempo para que el brazo deje de vibrar y la cámara enfoque
            time.sleep(1.0) 

            # ──────────────────────────────────────────────────────────
            # FASE 1.5: LECTURA DE PRECISIÓN (Cámara a 90° sobre la tapa)
            # ──────────────────────────────────────────────────────────
            self.get_logger().info('🔍 Tomando nueva lectura de profundidad centrada...')
            with self._lock:
                self._last_point = None # Limpiamos la lectura vieja
            
            # Esperamos hasta 2 segundos a que el nodo de visión mande la nueva medición
            t_start = time.time()
            punto_exacto = None
            while time.time() - t_start < 2.0:
                with self._lock:
                    if self._last_point is not None:
                        punto_exacto = self._last_point
                        break
                time.sleep(0.05)
                
            if punto_exacto is None:
                self.get_logger().error('❌ Se perdió de vista el cubo al centrar la cámara.')
                return False

            # Recalculamos la pose absoluta. Ahora 'z_obj' es la altura VERDADERA de la tapa.
            resultado_exacto = self._calcular_pose_objeto(punto_exacto)
            if not resultado_exacto: return False
            x_obj, y_obj, z_obj = resultado_exacto

            self.get_logger().info(f'📏 Distancia Cámara -> Tapa medida: {punto_exacto.z:.4f} m')
            self.get_logger().info(f'📏 Altura absoluta de la tapa (Z Base): {z_obj:.4f} m')

            # ──────────────────────────────────────────────────────────
            # FASE 2: PRE-PICK EXACTO
            # ──────────────────────────────────────────────────────────
            self.get_logger().info('>< FASE 2: Alineando gripper (Pre-Pick)...')
            ...
            # ──────────────────────────────────────────────────────────
            # FASE 3: PICK (La bajada crítica)
            # ──────────────────────────────────────────────────────────
            self.get_logger().info('👇 FASE 3: Bajando al cubo...')
            
            # El TCP busca estar exactamente Z_AGARRE_EXTRA_M por debajo de la tapa
            p_grip_pick = np.array([x_obj, y_obj, z_obj - Z_AGARRE_EXTRA_M])
            p_brida_pick = p_grip_pick - R_target @ t_grip

            self.get_logger().info(f'>> Target TCP Z: {p_grip_pick[2]:.4f} m | Target Brida Z: {p_brida_pick[2]:.4f} m')

            if self._plan_pose(p_brida_pick[0], p_brida_pick[1], p_brida_pick[2], quat, 'PICK'):
                self._exec_plan()
            else:
                self.get_logger().error('🚨 MoveIt rechazó la bajada. Posible colisión con la mesa o límites en Z.')
                return False

            # ──────────────────────────────────────────────────────────
            # FASE 4 y 5: CERRAR Y SUBIR
            # ──────────────────────────────────────────────────────────
            ...
            # Verificación de que el cubo ya no está en la mesa
            with self._lock:

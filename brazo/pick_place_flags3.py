...
#PLATFORM_JOINTS     = [-1.5708, -0.610865, -1.32645, 2.02458, 0.0] #Para ver cubos en plataforma

POSICIONES_PLATAFORMA = [
    [-2.0944, -0.628319, -0.174533, 0.802851, 0.0],       # 0: Izquierda
    [-1.5708, -0.698132, -0.148353, 0.8813913, 0.0],      # 1: Centro
    [-1.16937, -0.628319, -0.174533, 0.802851, 0.0]       # 2: Derecha
]
...
class PickAndPlaceNode(Node):
    def __init__(self):
        ...
        self._arm_joint_plan = self.create_client(
            PlanJoint,
            '/xarm_joint_plan',
            callback_group=self._cbg
        )
        ...
        self._sub_aruco = self.create_subscription(
            PointStamped,
            'coordenadas_arucos',
            self._vision_aruco_cb,
            10,
            callback_group=self._cbg
        )

        # Suscriptor de Bandera de Estación
        self._sub_bandera = self.create_subscription(
            String,
            'bandera_estacion',
            self._bandera_cb,
            10,
        ...
        # ────────────────────────────────────────────────────────
        # LLM: Suscriptor del color objetivo enviado por Gemini
        # Espera mensajes tipo:
        # {"color": "RED", "order_id": "123_RED_abc"}
        # También mantiene compatibilidad con mensajes tipo: RED
        # ────────────────────────────────────────────────────────
        target_color_qos = QoSProfile(
            depth=1,
        ...
        # Variables para evitar repetir la misma orden persistente
        self._orden_id_actual = None
        self._ultima_orden_completada = None
    ...
    # ────────────────────────────────────────────────────────
    # LLM: callback para recibir color desde /target_color
    # ────────────────────────────────────────────────────────
    def _parse_target_color_msg(self, msg: String):
        raw = msg.data.strip()
        ...
        try:
            ...
            if not order_id:
                order_id = f"no_id_{color}"

            return color, order_id

        except Exception:
            # Compatibilidad con mensajes manuales tipo "RED"
            color = raw.lower()
            order_id = f"manual_{color}"
            return color, order_id

    def _target_color_cb(self, msg: String):
        color, order_id = self._parse_target_color_msg(msg)

        if color not in COLORES_VALIDOS:
            self.get_logger().warn(
                f'Color recibido desde LLM no válido: "{color}"'
            )
            return

        with self._lock:
            if order_id == self._ultima_orden_completada:
                self.get_logger().warn(
                    f'Orden ya completada en pick_place. Se ignora order_id={order_id}'
                )
                return

            if self._color_objetivo is not None:
                self.get_logger().warn(
                    f'Ya hay una orden activa: {self._color_objetivo.upper()}. '
                    f'Se ignora nueva orden: {color.upper()} | order_id={order_id}'
                )
                return

            self._color_objetivo = color
            self._orden_id_actual = order_id
            self._last_cubo_msg = None

        self.get_logger().info(
            f'Orden recibida desde LLM: recolectar cubos {color.upper()} | order_id={order_id}'
        )
    ...
    def _hilo_logica(self):
        ...
        while rclpy.ok():
            # 1. ESPERAR COLOR DESDE LLM
            if self._color_objetivo is None:
                self.get_logger().info(
                    'Esperando orden de color desde LLM en /target_color...'
                )
                time.sleep(0.5)
                continue

            self.get_logger().info(
                f'Orden activa: {self._color_objetivo.upper()}. '
                'Esperando que el carro llegue a la ESTACIÓN DE PICK...'
            )
            ...
            self.get_logger().info(
                '¡Llegamos a la ESTACIÓN DE PICK! Iniciando carga del carrito...'
            )
            ...
            if cubos_cargados == 0:
                self.get_logger().info(
                    'No se cargó ningún cubo. Se aborta la orden actual.'
                )

                self._ultima_orden_completada = self._orden_id_actual
                self._orden_id_actual = None
                self._color_objetivo = None
            ...
            self.get_logger().info(
                f'Carga completada ({cubos_cargados} cubos). '
                'Preparando postura de viaje...'
            )
            ...
            self.get_logger().info(
                'Esperando que el carro viaje a la ESTACIÓN DE PLACE...'
            )
            ...
            self.get_logger().info(
                '¡Llegamos a la ESTACIÓN DE PLACE! Iniciando descarga...'
            )
            ...
            self.get_logger().info(
                'Descarga completada. Preparando postura de viaje...'
            )
            ...
            # 4. REINICIAR LOTE
            self.get_logger().info(
                'Lote de trabajo completamente procesado y entregado.'
            )
            ...
            with self._lock:
                self._estacion_actual = 'completado'
    ...
    def _rutina_estacion_place(self, cantidad_cubos: int):
        """Fase 2: Toma los cubos de la plataforma (por Joints fijos) y los pone sobre el ArUco"""
        cubos_descargados = 0

        # Transición segura: de HOME de viaje a HOME operativo antes de iniciar movimientos
        self._ir_a_posicion_articular(HOME_JOINTS)
        ...
        for iteracion in range(cantidad_cubos):
            ...
            # 1. Bajar a la posición exacta de la plataforma a ciegas (0: Izquierda, 1: Centro, 2: Derecha)
            pos_objetivo_plataforma = POSICIONES_PLATAFORMA[iteracion]

            self._ir_a_posicion_articular(PRE_PLATFORM_JOINTS)
            self._ir_a_posicion_articular(pos_objetivo_plataforma)

            # 2. Cerrar el gripper para tomar el cubo
            self._mover_gripper(cerrar=True)
            time.sleep(0.5)

            # 3. Subir el brazo (escape seguro vertical)
            self._ir_a_posicion_articular(PRE_PLATFORM_JOINTS)

            # 4. Ir a la pose de escaneo para buscar el ArUco de la estación de Place
            self._ir_a_posicion_articular(SEARCH_ARUCO_JOINTS)

            punto_aruco = self._esperar_deteccion_aruco(TARGET_ARUCO_ID)

            if punto_aruco:
                # Si ve el ArUco, hace el cálculo cartesiano para dejar el cubo y suma al contador
                ...
                self.get_logger().error(
                    'No se encontró el ArUco. Soltando cubo por seguridad.'
                )
                self._mover_gripper(cerrar=False)
                self._ir_a_posicion_articular(SEARCH_ARUCO_JOINTS)
    ...
    # ────────────────────────────────────────────────────────
    def _esperar_cubo(self, timeout=5.0):
        t_espera = 0.0
    ...
    def _ciclo_pick(self, punto, max_intentos=3) -> bool:
        for intento in range(1, max_intentos + 1):
            ...
            if not exito_lift:
                self.get_logger().error(
                    'Imposible levantar el brazo (Singularidad absoluta).'
                )
                return False
    ...
    def _ciclo_place_elevado(self, punto_aruco, offset_iteracion) -> bool:
        ...
        offset_dinamico_x = PLACE_OFFSET_X_BASE + (
            offset_iteracion * PLACE_OFFSET_X_INC
        )
        ...
        if not exito_lift:
            self.get_logger().error(
                'Imposible levantar el brazo (Singularidad).'
            )
            return False
